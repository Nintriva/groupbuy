        ...
        date_default_timezone_set($tz[$deal->timezone]); 
              ...
              if($deal->isTipped())   
                {
                  ...
                  foreach($deal->paid_transactions as $t)
                   ...
                   {    
                    ...
                    {
                      $deal->mailBuyers("ended_with_tipped");
               ...
               {         
                  $deal->status=4;
                  $deal->save(false);  
                  $deal->removeAllBuyers(); 
                  $deal->markAllCodesAreUnUsed(); 
                 // $deal->assignCodesToOffDealBuyers();
                    ...
                    //for free coupon buyers       
                    $deal->mailBuyers("expired_without_tipped1");                    
                ...
                foreach($deal->paid_transactions as $t)
                   {
                    ...
                    $deal->mailBuyers("expired_without_tipped");
                    ...
                    if($total_amt>0.0)
                     ...
                     else
                      {
                            //All the coupons were free  
                                 ...
                                 $deal->mailBuyers("sold_out_before_exp");
         ...
         else if($deal->is_expired==1&&$deal->isExpired())
          {
             ...
             echo "Expired".$deal->title."<br/>";
               ...
               foreach($deal->coupons as $c)
                  {
                    if($c->status!=3&&$c->status!=5&&$deal->isCouponExpired())
                      {
                        $c->status=5;//expired
                        $c->save();  
                         //$deal->RedeemCoupon($c);
               ...
               // echo "<br/>".$deal->title."<br/>";
                //echo (strtotime($deal->end_date)+((60*60*24)*$deal->days_in_todays_deal_after_exp));
                //echo "<br/>".mktime()."<br/>";
               if((strtotime($deal->end_date)+((60*60*24)*$deal->days_in_todays_deal_after_exp))<=mktime()&&$deal->status!=8&&$deal->status!=5)
                 {
                   echo "<br/>Movedddddd:".$deal->title;  
                   $deal->status=8;
                   $deal->save(false);  
                 }
